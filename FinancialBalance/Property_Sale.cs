...
{
    public partial class Property_Sale : Form
    {
        ...
        private void Property_Sale_Load(object sender, EventArgs e)
        {
            Filling = true;
            Fill_Dates();
            Fill_Property();
        ...
        private void Clear_Grid()
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                gvSale.Columns[i].HeaderCell.Style.Alignment = TmpAlign;
                gvSale.Columns[i].DefaultCellStyle.Alignment = TmpAlign;
            }
        }
        ...
        private void Get_Data()
        {
            try
            {
                ...
                Mdl1.Ssql = "select " + Joined_Fields + ", r.[Name] as Property_Name"
                          + " from TblPropertySale as p left join TblProperty as r"
                ...
                while (reader.Read())
                {
                    ...
                    gvSale.Rows[gvSale.Rows.Count - 1].Tag = Read_Text(reader["Property_Id"]);
                }
                reader.Close();
        ...
        private void Load_Record(int parId)
        {
            try
            {
                Mdl1.Ssql = "select " + Fields + " from TblPropertySale"
        ...
        {
            return new TextBox[] { txtSoldPrice, txtConveyancing, txtSaleAgent, txtSettlement,
                                   txtOtherCost };
        }
                                ...
                                out string parWhy)
        {
            ...
            {
                parWhy = "Sold Date needs a day, a month and a year.";
                return false;
            }
            if (!Mdl1.k_Date(TmpDD + TmpMM + TmpYear))
            {
                parWhy = "Sold Date is not a real date.";
                return false;
            }
        ...
        {
            Mdl1.Ssql = "select [Property_Id] from TblPropertySale where [Property_Id] = "
        ...
        private void CmdCreate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //one sale per property, so a second is an update rather than another row
                if (Exists(TmpId))
                {
                    MessageBox.Show("Property Id " + TmpId.ToString(CultureInfo.InvariantCulture)
                        + " already has a sale record." + Environment.NewLine
        ...
        private void CmdUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Property Id " + TmpId.ToString(CultureInfo.InvariantCulture)
                        + " already has a sale record.", "Error Message");
                    return;
                }
                if (!Exists(OrgId))
                {
        ...
        private void CmdDel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DialogResult Response = MessageBox.Show(
                    "Delete the sale record for Property Id "
                ...
                Mdl1.Ssql = "Delete from TblPropertySale where [Property_Id] = "
                          + OrgId.ToString(CultureInfo.InvariantCulture);
                OleDbCommand cmd = new OleDbCommand(Mdl1.Ssql, Mdl1.conn);
                cmd.ExecuteNonQuery();
