...
It holds build output beside the real database and an `.exe.config` carrying live Google
credentials, none of which belongs in the repository — and since `bin/` is tracked here, an
unqualified `git add -A` would otherwise sweep those credentials in. Deploying to it means
copying `bin/Release/FinancialBalance.exe` over the one already there **and nothing else**: the
`.mdb` and the `.exe.config` beside it are live state rather than build output, and the copies
...
list** rather than on the blank, so the page has something on it the moment it is reached. The
blank stays at the top for a deliberate nothing-selected, and is all there is to pick when no
property has been set up yet.

**Address** is `Address`, `Suburb`, `State` and `Post_Code` run together with single spaces. A
part left empty is dropped rather than spaced over, so a property with no unit number does not
